...
| `agent_tools.py` | **(network; consumer side)** A dependency-light `MandatehubClient` (discover / preview / purchase) plus **LangChain** and **CrewAI** tool adapters (`langchain_tools()`, `crewai_tools()`). Framework imports are optional/guarded. Run it directly for a free, no-key catalog + preview demo. |
| `mandatehub_quickstart.ipynb` | **(network; free, no wallet)** Colab/Jupyter quickstart: discover the catalog, preview 402 terms, read the OpenAPI spec — all free. The final cell shows the one-liner that would actually pay. |
